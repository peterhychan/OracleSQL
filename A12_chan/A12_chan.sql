...
, BOOK_ID AS "Book ID"
, TITLE AS "Book_Title"
, TO_CHAR(QUANTITY_OF_BOOKS, '999,999') AS "TotQuantSale"
FROM BOOK_SALES 
...
/*  TASK 04 */ 
COLUMN "RunningSalesByCustomer" FROMAT 999,999.99

WITH ANNUAL_SALES AS
	(SELECT CUST_ID
	, TO_CHAR(ORDER_DATE, 'YYYY-MM-DD') AS DATE_OF_ORDERS 
	, SUM(NVL(QUANTITY, 0) * NVL(ORDER_PRICE, 0)) AS SALES_IN_TOTAL
...
, TO_CHAR(SALES_IN_TOTAL, '999,999.99') AS "SalesByDay"
, SUM(SALES_IN_TOTAL) OVER (PARTITION BY CUST_ID ORDER BY DATE_OF_ORDERS) AS "RunningSalesByCustomer"
FROM ANNUAL_SALES 
...
WITH CALENDAR AS
	(SELECT TO_DATE(&START_DATE, 'YYYY-MM-DD') - 2 + ROWNUM AS CAL_DATE
	FROM BK_ORDER_DETAILS
...
, DAILY_SALES_RECORDS AS
	...
	GROUP BY CAL_DATE)
SELECT TO_CHAR(ORD_DATE, 'YYYY-MM-DD') AS "Date"
...
WHERE EXTRACT(MONTH FROM ORD_DATE) = 3
ORDER BY 1
;
